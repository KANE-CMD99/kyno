    ...
    <div>
      <div className="mb-6 flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-neutral-500">{users.length} registered</p>
        </div>
      ...
      {showForm && (
        <div className="mb-6 rounded-xl border border-neutral-200 bg-white p-6">
          ...
          <div className="mt-5 flex gap-3">
            <button onClick={handleSubmit} disabled={!name || !email || (!editingId && !password)} className="rounded-lg bg-neutral-900 px-6 py-2.5 text-sm font-semibold text-white transition-colors hover:bg-neutral-800 disabled:opacity-50">
      ...
      {loading ? (
        ...
        <div className="overflow-hidden rounded-xl border border-neutral-200 bg-white">
          <table className="w-full text-left text-sm">
            ...
            <tbody>
              {users.length === 0 ? (
                <tr><td colSpan={4} className="px-5 py-12 text-center text-neutral-400">No users registered yet.</td></tr>
              ) : (
                users.map((u) => (
