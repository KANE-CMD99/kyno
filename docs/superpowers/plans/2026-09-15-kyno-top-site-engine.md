...
- **`http://www.kyno.top/` served 200 over plaintext — FIXED.** Cloudflare → **SSL/TLS → Edge Certificates → Always Use HTTPS** was enabled, and it is verified live: all four HTTP forms (`http://www.kyno.top/`, a deep path, `http://kyno.top/`, and a deep apex path) return **301** with the path preserved, and following each reaches the HTTPS page in a single hop. `http://kyno.top/…` collapses the protocol upgrade *and* the apex→www redirect into that one 301, so no chain forms. The HTTPS side is unchanged and HSTS is still present. This was the finding with real weight — content was being served unencrypted.
- **Trailing-slash redirects are `307`, not `301` — the one item still open.** `html_handling = "drop-trailing-slash"` was tried and **did not change the status code** — Cloudflare normalises assets with a temporary redirect regardless of the mode. Getting a permanent one needs a zone Redirect Rule: when `http.host eq "www.kyno.top" and http.request.uri.path matches "^/.+/$"`, then Dynamic → `concat("https://www.kyno.top", regex_replace(http.request.uri.path, "/$", ""))`, status 301. The destination is already correct and crawlers follow 307s, so this is polish.
...
**Ads — re-confirmed as deferred, on 2026-09-16, the day the site went live.** AdSense was offered and declined: the spec puts ads in phase 2 gated on "only once pages hold real rankings", and that gate is not met — the domain had zero backlinks and no index presence on launch day, so revenue would have been ~$0 while the "made for ads" signal applies immediately. **The absence of an AdSense script in `layout.tsx` is deliberate, not an omission.** If it is ever added, the same change must rewrite `/privacy` — it currently states "No cookies" and "No tracking pixels", which AdSense would make false — and should reuse the consent-gated loading pattern the sibling site at kynocreative.com already uses.
